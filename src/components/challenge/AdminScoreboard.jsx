...
import { X, Download, Trophy, RefreshCw, Shield } from 'lucide-react';
...
export default function AdminScoreboard() {
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (e) => {
      ...
      if (!'scoreboard'.startsWith(typed)) {
        typed = e.key.toLowerCase();
      }
      if (typed === 'scoreboard') {
        e.preventDefault();
  ...
  const exportCSV = () => {
    if (!data) return;
    const headers = ['Rank', 'Namn', 'E-post', 'Veckopoäng', 'Totalpoäng'];
    ...
    a.href = url;
    a.download = `anchor-challenge-${new Date().toISOString().split('T')[0]}.csv`;
    a.click();
          ...
          >
            ...
            <div className="p-5 border-b border-white/10 flex items-center justify-between flex-shrink-0">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-400">{data?.total_participants || 0} deltagare registrerade</p>
                </div>
              </div>
            ...
            <div className="overflow-y-auto flex-1 p-4 space-y-2">
              {loading ? (
                ...
                <div className="text-center py-12 text-slate-500">Inga deltagare ännu</div>
              ) : (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div className="text-right flex-shrink-0">
                        <p className="font-bold text-indigo-300 text-lg">{(entry.weekly_points || 0).toLocaleString('sv-SE')}</p>
                        <p className="text-xs text-slate-500">tot: {(entry.total_points || 0).toLocaleString('sv-SE')} p</p>
                      </div>
                    </div>

                    {/* Recent events (anti-cheat) */}
              ...
              {/* Rest of users (no recent events) */}
