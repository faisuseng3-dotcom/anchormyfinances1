// AvatarBuilder — "Lumis-3D" style builder matching the reference image
// Dark UI, centered avatar with aura, color wheel, Galaxy mini-list sidebar

import React, { useState, useCallback } from 'react';
import { motion, AnimatePresence } from 'framer-motion';
import { Shuffle, Check, Dice6 } from 'lucide-react';
...
// Icons as SVG paths (outline style, matching reference image)
const CAT_ICONS = {
  face: (
    <svg viewBox="0 0 24 24" className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round">
      <circle cx="12" cy="12" r="9" /><circle cx="9" cy="11" r="1.2" fill="currentColor" stroke="none" /><circle cx="15" cy="11" r="1.2" fill="currentColor" stroke="none" />
      <path d="M9 15.5 Q12 18 15 15.5" />
    </svg>
  ),
  hair: (
    <svg viewBox="0 0 24 24" className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round">
      <path d="M6 4 Q6 2 9 2 Q12 0 15 2 Q18 2 18 4 Q20 6 19 10 Q18 16 12 20 Q6 16 5 10 Q4 6 6 4Z" />
    </svg>
  ),
  eyes: (
    <svg viewBox="0 0 24 24" className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round">
      <path d="M1 12 Q6 6 12 6 Q18 6 23 12 Q18 18 12 18 Q6 18 1 12Z" /><circle cx="12" cy="12" r="3" />
    </svg>
  ),
  nose_mouth: (
    <svg viewBox="0 0 24 24" className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round">
      <path d="M11 9 Q10 14 8 15 Q10 16 12 16 Q14 16 16 15 Q14 14 13 9" /><path d="M8.5 19 Q12 22 15.5 19" />
    </svg>
  ),
  clothes: (
    <svg viewBox="0 0 24 24" className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round">
      <path d="M2 8 L8 5 Q10 8 12 8 Q14 8 16 5 L22 8 L20 14 L17 13 L17 22 L7 22 L7 13 L4 14 Z" />
    </svg>
  ),
  accessories: (
    <svg viewBox="0 0 24 24" className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round">
      <circle cx="7" cy="12" r="4" /><circle cx="17" cy="12" r="4" /><line x1="11" y1="12" x2="13" y2="12" /><line x1="3" y1="10" x2="1" y2="11" /><line x1="21" y1="10" x2="23" y2="11" />
    </svg>
  ),
};
...
const CATEGORIES = [
  ...
  },
  {
    ...
    colorKey: null,
    sections: [
      { label: 'Accessoar', type: 'avatar_preview_small', options: ACCESSORIES, stateKey: 'accessory' },
    ],
  },
];
...
export default function AvatarBuilder({ value, onChange, onSave, saved }) {
  ...
  // Color wheel target for current category
  ...
  return (
    ...
    >
      ...
      <div className="flex gap-0">
        ...
        <div className="flex-1 flex flex-col items-center justify-center pt-2 pb-0 relative">
          <AvatarDisplay config={config} size={160} />
        </div>

        {/* Right: Slumpa + Galaxy list */}
      ...
      {/* ── CATEGORY RIBBON ── */}
      <div className="flex items-center border-t border-b" style={{ borderColor: 'rgba(255,255,255,0.06)', background: 'rgba(0,0,0,0.2)' }}>
        {CATEGORIES.map(cat => {
        ...
        >
          {saved ? <Check className="w-5 h-5" /> : <Shuffle className="w-5 h-5" />}
          <span className="text-[8px] font-bold">{saved ? 'Sparad ✓' : 'Kläder'}</span>
        </motion.button>
      </div>

      {/* ── OPTION GRID ── */}
      <div className="flex-1 overflow-y-auto relative" style={{ background: '#0d1117' }}>
          ...
          >
            {currentCat?.sections.map(section => (
              <div key={section.stateKey}>
                ...
                {/* ── AVATAR PREVIEW GRID (large) ── */}
                {(section.type === 'avatar_preview' || section.type === 'avatar_preview_small') && (
                  <div className="px-2 pb-2">
                    <div className="grid grid-cols-4 gap-1.5">
                      {section.options.map((opt, idx) => {
                        ...
                        const isMiddle = idx === 4 || idx === 5; // center slots in row 2
                        ...
                        return (
                          ...
                          >
                            <_AvatarSVG config={previewConfig} size={56} />
                      ...
                      {/* Color wheel in the grid (after item 7) */}
                      {currentCat?.colorKey && section.type === 'avatar_preview' && (
                        <motion.div
                  ...
                  return (
                    <motion.button
                      ...
                      style={{
                        ...
                        boxShadow: active
                          ? `0 0 0 2px #111827, 0 0 0 4px ${col}, 0 6px 14px ${col}80`
