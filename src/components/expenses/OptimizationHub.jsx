...
import { X, Zap, ShoppingCart, Star } from 'lucide-react';
...
function BasketSwap({ item }) {
  ...
  return (
    <div>
      <p className="text-sm font-bold text-white mb-3">🛒 The Basket Swap</p>
...
function GhostNegotiator({ item }) {
  ...
  return (
    <div>
      <p className="text-sm font-bold text-white mb-3">👻 The Ghost Negotiator</p>
      ...
      {step >= 3 && (
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
          <div className="p-3 rounded-xl mb-4" style={{ background: 'rgba(99,102,241,0.1)', border: '1px solid rgba(99,102,241,0.2)' }}>
            <p className="text-xs text-indigo-300">✦ Ditt abonnemang kostar <span className="font-bold text-rose-400">{fmt(item.amount)} kr</span>. Snittet för din förbrukning ligger på <span className="font-bold text-emerald-400">{fmt(market)} kr</span>. Jag har förberett ett manus du kan klistra in hos din operatör.</p>
          </div>
...
function MicroHabitSwitch({ item }) {
  ...
  return (
    <div>
      <p className="text-sm font-bold text-white mb-3">☕ The Micro-Habit Switch</p>
      ...
      <div className="p-3 rounded-xl mb-4" style={{ background: 'rgba(99,102,241,0.1)', border: '1px solid rgba(99,102,241,0.2)' }}>
        <p className="text-xs text-indigo-300">✦ Om du brygger hemma halverar du kostnaden. Du når din <span className="font-bold text-white">Madrid-resa</span> {monthsEarlier} dagar tidigare.</p>
      </div>
...
function HappinessMap({ feelings, item }) {
  ...
  return (
    <div className="mt-5 pt-4 border-t border-white/10">
      <p className="text-sm font-bold text-white mb-3">💖 Livskvalitets-analys</p>
      <div className="grid grid-cols-3 gap-2">
        {[
          { label: 'Älskar', count: loved, color: 'text-yellow-400', bg: 'rgba(234,179,8,0.1)', emoji: '⭐' },
          { label: 'Okej', count: meh, color: 'text-slate-400', bg: 'rgba(100,116,139,0.1)', emoji: '😐' },
          { label: 'Onödig', count: skip, color: 'text-rose-400', bg: 'rgba(239,68,68,0.1)', emoji: '💸' },
