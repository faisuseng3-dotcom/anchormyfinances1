  ...
  return (
    <AnimatePresence>
      <motion.div
        ...
        animate={{ opacity: 1 }}
        exit={{ opacity: 0 }}
        className="fixed inset-0 anchor-overlay z-50 flex items-end"
        onClick={onClose}
      >
        <motion.div
          initial={{ y: '100%' }}
          ...
          className="w-full max-w-lg mx-auto rounded-t-[28px] shadow-2xl max-h-[min(85dvh,720px)] flex flex-col ring-1 ring-inset ring-[var(--color-border)]"
          style={{ background: '#FFFFFF' }}
        >
          <div className="p-6 pb-4 border-b border-[var(--color-border)] flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-2xl bg-[#2563EB]/25 ring-1 ring-[#2563EB]/30 flex items-center justify-center">
                <Radio className="w-5 h-5 text-[var(--color-accent)]" />
              </div>
              <div>
                <h2 className="font-semibold text-[var(--color-text-primary)]">Röstrådgivare</h2>
            ...
            <Button
              ...
              size="icon"
              onClick={onClose}
              className="rounded-full text-[var(--color-text-secondary)] hover:text-[var(--color-text-primary)] hover:bg-[var(--color-background-secondary)]"
            >
              <X className="w-5 h-5" />
            </Button>
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-4">
            {messages.length === 0 && (
              <div className="text-center py-8">
                <div className="w-16 h-16 rounded-2xl bg-[var(--color-background-secondary)] ring-1 ring-[var(--color-border)] flex items-center justify-center mx-auto mb-4">
                  <Mic className="w-8 h-8 text-[var(--color-accent)]" />
                </div>
                <h3 className="font-semibold text-[var(--color-text-primary)] mb-2">Prata med Lago</h3>
                ...
                <div
                  className={`max-w-[80%] rounded-2xl px-4 py-3 ${
                    message.role === 'user'
                      ...
                      : 'bg-[var(--color-background-secondary)] ring-1 ring-[var(--color-border)] text-[var(--color-text-primary)]'
                  }`}
                >
            ...
            {isProcessing && (
              ...
              >
                <div className="bg-[var(--color-background-secondary)] ring-1 ring-[var(--color-border)] rounded-2xl px-4 py-3">
            ...
            {transcript && isListening && (
              ...
              >
                <div className="bg-[#2563EB]/20 ring-1 ring-[#2563EB]/20 rounded-2xl px-4 py-3 max-w-[80%]">
                  <p className="text-sm text-[var(--color-text-secondary)]">{transcript}</p>
                </div>
              </motion.div>
            )}
          ...
          <div className="p-6 pt-4 border-t border-[var(--color-border)]">
              ...
              >
                <motion.div
                  ...
                  className={`absolute inset-0 rounded-full ${
                    isListening ? 'bg-[#2563EB]/50' : 'bg-[#2563EB]/60'
                  } blur-xl`}
                />
                <motion.div
                  whileTap={{ scale: 0.95 }}
                  className={`relative w-16 h-16 rounded-full flex items-center justify-center ${
                    isListening
                      ? 'bg-[var(--color-accent)]'
                      : 'bg-[var(--color-accent)]'
                  } shadow-lg ring-2 ring-[#2563EB]/30`}
                >
                  {isProcessing ? (
            ...
            {!recognitionRef.current && (
              <p className="text-xs text-center text-[var(--color-warning)] mt-3">
