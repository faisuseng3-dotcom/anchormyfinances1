...
function ChallengeRow({ challenge, transactions }) {
  ...
  return (
    <div className="px-4 py-4 border-b border-[var(--color-border)] last:border-0">
      <div className="flex items-start gap-3">
        <div
          className={`w-2 h-2 rounded-full flex-shrink-0 mt-2 ${done ? 'bg-[var(--color-success)]' : 'bg-[var(--color-accent)]'}`}
        />
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between gap-2 mb-0.5">
            <p className={`text-sm font-semibold ${done ? 'text-[var(--color-success)]' : 'text-[var(--color-text-primary)]'}`}>
              {challenge.title}
            </p>
            <span
          ...
          <div className="flex items-center gap-2">
            ...
            <p className="text-xs font-semibold text-[var(--color-text-secondary)] flex-shrink-0 tabular-nums">{progress.label}</p>
          </div>
          {done && <p className="text-xs font-semibold text-[var(--color-success)] mt-1">Bra jobbat — utmaningen klar!</p>}
        </div>
      </div>
    </div>
  );
    ...
    <div className="space-y-4">
      <div className={`${surface.card} p-5`}>
        <div className="flex items-center gap-4 mb-4">
          <div className="w-14 h-14 rounded-[var(--anchor-radius-lg)] flex items-center justify-center flex-shrink-0 bg-gradient-to-br from-[var(--color-accent)] to-[var(--color-success)] anchor-elev-2">
            <span className="text-2xl font-light text-white tabular-nums">{level}</span>
          </div>
          <div>
            <p className="anchor-type-body-sm text-[var(--color-text-secondary)]">Din nivå</p>
            <p className="text-xl font-semibold text-[var(--color-text-primary)] tracking-tight">Level {level}</p>
      ...
      {badges.length > 0 && (
        <div className={`${surface.card} p-5`}>
          <p className="anchor-type-body-sm text-[var(--color-text-secondary)] mb-3">Dina badges</p>
          <div className="flex flex-wrap gap-3">
            {badges.map((b) => (
              <div key={b} className="flex flex-col items-center gap-1">
                <div className="w-12 h-12 rounded-[var(--anchor-radius-lg)] flex items-center justify-center bg-[var(--color-success-soft)] ring-1 ring-[rgba(22,163,74,0.2)] anchor-elev-1">
                  <Star className="w-5 h-5 text-[var(--color-success)]" />
                </div>
                <p className="text-xs text-center max-w-12 leading-tight text-[var(--color-text-secondary)]">{b.replace(/_/g, ' ')}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className={`${surface.card} overflow-hidden p-0`}>
        <div className="px-4 py-3 border-b border-[var(--color-border)] flex items-center gap-2">
          ...
          <p className="anchor-type-body-sm text-[var(--color-text-secondary)]">Aktiva utmaningar</p>
        </div>
        {CHALLENGES.map((ch) => (
  ...
  return (
    <div className={`${surface.card} p-5`}>
      <div className="flex items-center gap-2 mb-3">
        ...
        <p className="anchor-type-body-sm text-[var(--color-text-secondary)]">Mot dig själv</p>
      </div>
      <div className="grid grid-cols-2 gap-3 mb-3">
        <div className="rounded-[var(--anchor-radius-md)] p-3 bg-[var(--color-background-secondary)] ring-1 ring-[var(--color-border)]">
          <p className="anchor-type-body-sm mb-1">Förra månaden</p>
          <p className="text-lg font-semibold text-[var(--color-text-primary)] tabular-nums">{lastMonth.toLocaleString('sv-SE')} kr</p>
        </div>
        <div
          className={`rounded-[var(--anchor-radius-md)] p-3 ring-1 ${
            better
              ? 'bg-[var(--color-success-soft)] ring-[rgba(22,163,74,0.2)]'
              : 'bg-[var(--color-danger-soft)] ring-[rgba(220,38,38,0.2)]'
          }`}
        >
          <p className="anchor-type-body-sm mb-1">Denna månad</p>
          <p className={`text-lg font-semibold tabular-nums ${better ? 'text-[var(--color-success)]' : 'text-[var(--color-danger)]'}`}>
            {thisMonth.toLocaleString('sv-SE')} kr
          </p>
        </div>
      </div>
      <p className={`text-sm font-medium leading-relaxed ${better ? 'text-[var(--color-success)]' : 'text-[var(--color-danger)]'}`}>
        {better
